        ...
        <el-form :inline="true" :model="tableForm" class="demo-form-inline form-inline">
          <div class="form-inline-left">
            ...
            <el-form-item label="上下行方向">
              <el-select size="small" v-model="tableForm.sxxfx" placeholder="请选择" clearable>
                <el-option
                  v-for="item in sxxfxOption"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
            ...
            <el-form-item label="服务区编号">
              <el-select size="small" v-model="tableForm.id" placeholder="请选择">
                <el-input
                  placeholder="请输入"
  ...
  export default {
    ...
    data () {
      return {
        ...
        value: '',
        sxxfxOption: config.range.sxxfxOption,
    ...
    methods: {
      ...
      handClick (type, item) {
        switch (type) {
          ...
          case 'edit':
            ...
            this.$router.push({name: 'serviceAreaEdit', params})
            break
        }
      },
      /**
      ...
      removeHand (item) {
        this.$confirm('确认删除该信息吗?', '提示', {
          ...
          item['operation'] = 3
          this.$api.removeServiceArea(item).then(res => {
            if (res.status === 200) {
              this.$message({
                message: '删除成功！',
                type: 'success'
      ...
      * */
      onSeach () {
        ...
        this.$api.loadServiceAreaList(this.tableForm).then(res => {
      ...
      formatterDate (row, column, cellValue, index) {
        return filters.formatDate(cellValue)
      },
