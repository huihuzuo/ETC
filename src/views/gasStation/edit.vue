<template>
  <div class="query-form-wrap">
    ...
    <div class="query-form-wrap-content">
      <el-form :model="form" label-width="100px" :label-position="'left'" ref="gasStationRules" :rules="gasStationRules" size="small">
        <div class="form-content-scroll-edit">
          <span class="query-form-wrap-content-title">基本信息</span>
          <el-form-item label="加油站编号" prop="name">
            <el-input v-model="form.id" disabled></el-input>
          </el-form-item>
          <el-form-item label="加油站名称" prop="name">
            <el-input v-model="form.name" placeholder="请输入加油站名称"></el-input>
          </el-form-item>
          <el-form-item label="所属运行方编号">
            <el-input v-model="form.gasStationOperatorId" placeholder="请输入所属运行方编号"></el-input>
          </el-form-item>
          <el-form-item label="加油站地址">
            <el-input v-model="form.address" placeholder="请输入加油站地址"></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      goback () {
        this.$router.push('/gasStationList')
      },
      viewPointLocation () {
      },
      handClick (lng, lat) {
      ...
      monitorPoint (event, coordinates) {
        ...
        let point = this.addPoint(obj, require('../../../static/images/icon/icon_map_gasStation.png'), require('../../../static/images/icon/icon_map_gasStation_hover.png'))
      ...
      loadDetailById (id) {
        this.$api.queryGasStationDetailById(id).then(res => {
      ...
      onEdit () {
        this.$refs.gasStationRules.validate((valid) => {
          if (valid) {
            this.$api.editGasStation(this.form).then(res => {
              if (res.status === 200) {
                this.$message({
                  message: '编辑成功！',
                  type: 'success'
                })
                this.$router.push({ name: 'gasStationList' })
              } else {
