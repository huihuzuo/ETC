          ...
          <div class="result-wrap-content-top">
            <div @mouseover="mouseover(item, $index)" @mouseout="mouseout(item, $index)" class="result-list-item-div" v-for="(item, $index) in data" @click="handleClick(item)" :class="currentSelect === item.id ? 'result-list-currentSelect' : ''">
              ...
              <div class="item-right">
                ...
                <div class="item-right-second" :title="item.address">{{item.address}}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    methods: {
      ...
      queryResultList () {
        ...
        let _params = {
          ...
          id: this.queryParams && this.queryParams._params && this.queryParams._params.id ? this.queryParams._params.id : null, // 服务网点编号
          name: this.queryParams && this.queryParams._params && this.queryParams._params.name ? this.queryParams._params.name : null, // 服务网点名称
          // serviceItems: this.queryParams && this.queryParams._params && this.queryParams._params.serviceItems ? this.queryParams._params.serviceItems : null, // 服务项目
          orgType: this.queryParams && this.queryParams._params && this.queryParams._params.orgType ? this.queryParams._params.orgType : null, // 合作机构类别
          operations: operations
        }
        this._params = _params
        this.$api.loadETCServerList(_params).then(res => {
