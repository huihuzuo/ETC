      ...
      <el-form ref="form" :model="form" label-width="100px" :label-position="'left'" size="small">
        <div class="form-content-scroll">
          ...
          <el-form-item label="收费站类型">
            <el-select v-model="form.type" placeholder="请选择收费站类型" clearable>
              <el-option v-for="item in tollStationType" :label="item.label" :value="item.value" :key="item.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="收费站广场类型">
            <el-select v-model="form.plazaType" placeholder="请选择收费站广场类型" clearable>
              <el-option v-for="item in plazaType" :label="item.label" :value="item.value" :key="item.value"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="收费站广场编码">
            <el-input v-model="form.plazaId" placeholder="请输入收费站广场编码" clearable></el-input>
          </el-form-item>
          ...
          <el-form-item label="收费站编码">
            <el-input v-model="form.stationid" placeholder="请输入收费站编码" clearable></el-input>
          </el-form-item>
        </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        yesOrNo: config.range.yesOrNo, // 收费广场车道数与收费车道数据是否相符等条件select
        plazaType: config.range.plazaType, // 收费站广场类型select
        form: {
          name: null, // 收费站名称
          roadcode: null, // 路线编码
          roadname: null, // 路线名称
          division: null, // 行政区划
          type: null, // 收费站类型
          plazaType: null, // 收费广场类型
          plazaId: null, // 收费广场编码
          plazaName: null, // 收费广场名称
          stationid: null // 收费站编码
          // 具体条件待接口确认后再修改
        }
      }
    },
