...
<script>
  ...
  const _layerName = 'tollStation' // 停车场LayerName
  export default {
    props: {},
    data () {
      return {
    ...
    methods: {
      ...
      handClick (type, item) {
        switch (type) {
          ...
          case 'edit':
            ...
            this.$router.push({ name: 'parkEdit', params })
            break
        }
      },
      /**
      ...
      removeHand (item) {
        this.$confirm('确认删除该信息吗?', '提示', {
          ...
          item['operation'] = 3
          this.$api.removePark(item).then(res => {
            if (res.status === 200) {
              this.$message({
                message: '删除成功！',
                type: 'success'
      ...
      /*
      * 查询列表方法，有loading服务方式的效果，数据返回来后渲染到页面，loading消失
      * */
      onSeach () {
        ...
        this.$api.loadParkList(this.tableForm).then(res => {
