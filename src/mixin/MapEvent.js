import _showPopover from '../components/popover/index'
...
export default {
  methods: {
    ...
    matchConfig (properties, coordinates) {
      if (properties.layerName) {
        switch (properties.layerName) {
          case 'tollStationSquare': // 收费广场
            coordinates = (coordinates instanceof Array) ? coordinates : formatWKT(coordinates)
            _showPopover('tollStationSquare', this.$Maps.getMap(), {
              id: properties['id'],
            ...
            break
          case 'serviceArea': // 服务区
            coordinates = (coordinates instanceof Array) ? coordinates : formatWKT(coordinates)
            _showPopover('serviceArea', this.$Maps.getMap(), {
              id: properties['id'],
              layerName: properties['layerName'],
              coordinates: coordinates,
              properties: properties
          ...
          case 'gasStation': // 加油站
            coordinates = (coordinates instanceof Array) ? coordinates : formatWKT(coordinates)
            _showPopover('gasStation', this.$Maps.getMap(), {
              id: properties['id'],
              layerName: properties['layerName'],
              coordinates: coordinates,
              properties: properties
          ...
          case 'park': // 停车场
            coordinates = (coordinates instanceof Array) ? coordinates : formatWKT(coordinates)
            _showPopover('park', this.$Maps.getMap(), {
              id: properties['id'],
              layerName: properties['layerName'],
              coordinates: coordinates,
              properties: properties
            ...
            break
          case 'tollStation': // 收费站
            coordinates = (coordinates instanceof Array) ? coordinates : formatWKT(coordinates)
            _showPopover('tollStation', this.$Maps.getMap(), {
              id: properties['id'],
              layerName: properties['layerName'],
              coordinates: coordinates,
              properties: properties
    ...
    getProperties (type, value) {
      ...
      if (type === 'feature:onselect') {
        ...
        if ($LY) {
          ...
          let $CF = config.getLayerConfigByLayerName(properties['layerName'], 'layers')
          if ($CF) {
            if (value.getId()) {
    ...
    drawPoint () {
      this.$Maps.removeFeatureByLayerName('drawPoint')
      if (this.drawObj) {
        this.$Maps.removeInteraction(this.drawObj)
      }
      this.drawObj = new ol.interaction.Draw({
        type: 'Point',
        freehand: true
      })
      this.$Maps.addInteraction(this.drawObj)
      return this.drawObj
    },
    addPoint (obj, img, selectImg) {
      let point = this.$Maps.addPoint(obj, {
        layerName: 'drawPoint',
        zoomToExtent: false,
        selectable: true,
        moveable: true,
