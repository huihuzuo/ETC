<!--收费站广场结果列表-->
<template>
  <div class="identificPointResultList">
    <div @mouseover="mouseover(item, $index)" @mouseout="mouseout(item, $index)" class="result-list-item-div" v-for="(item, $index) in tollStationSquareData" @click="handleClick(item)" :class="currentSelect === item.id ? 'result-list-currentSelect' : ''">
      ...
      <div class="item-right">
        ...
        <div class="item-right-second" :title="item.plazaType | rangeConversion(plazaType)">{{item.plazaType | rangeConversion(plazaType)}}</div>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  const _layerName = 'identificPointResult' // 标识点图层LayerName
  export default {
    props: {
      // 标识点数据
